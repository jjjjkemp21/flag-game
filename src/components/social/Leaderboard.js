...
import TitleBadge from '../profile/TitleBadge';
...
function Leaderboard({ setView, flagsData }) {
    ...
    const load = useCallback(async (silent = false) => {
        if (!isAuthed) return;
        // Monotonic request id: a slow response for an old scope/filter must not
        // overwrite a newer one (rapid tab switching or a poll overlapping a
        // slow request), which would render rows/labels for the wrong scope.
        ...
        try {
            ...
            if (myReq === reqIdRef.current && !silent) setLoading(false);
        }
