...
import { ScoreBubble, ProgressRing } from '../ui/index';
...
function FrenzyQuiz({ allFlagsData, setView, strictSpelling = false }) {
    ...
    // Latched "beat your best" result, captured once at game-over BEFORE
    // highScore is updated — so the celebration banner/mascot don't flicker
    // away when setHighScore makes score === highScore on the next render.
    const [isNewHigh, setIsNewHigh] = useState(false);
    // One-shot guard so the end-of-run settlement (chest roll, Bucks credit,
    // quest bumps) runs exactly once per game-over even though the effect's
    // deps change when highScore updates.
    ...
    useEffect(() => {
        ...
        if (beatBest) {
            ...
            // Push the new high synchronously so the battlepass refresh reads it
            // instead of the previous (stale) bonus_scores_json.
        ...
        // approximate it from score relative to a "good run" threshold of 30 —
        // anything at or above that biases the chest toward higher rarities,
        // matching how the standard quizzes' accuracy boost works.
    ...
    const handleSubmit = (e, index) => {
        ...
        if (checkAnswer(slot.inputValue, slot.flag, strictSpelling)) {
            audio.play('correct');
        ...
        return (
            <div className="frenzy-notification-overlay">
                <motion.div
                    ...
                    animate="animate"
                >
                    <Mascot size={88} mood={isNewHigh ? 'cheer' : 'sad'} />
                    <h2>Game Over!</h2>
