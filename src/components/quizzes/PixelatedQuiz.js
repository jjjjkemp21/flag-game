...
function PixelatedQuiz({ allFlagsData, setView, strictSpelling = false }) {
    ...
    // See FrenzyQuiz: latch the "beat best" result so the celebration doesn't
    // flicker away, and guard end-of-run settlement so it runs exactly once.
    ...
    useEffect(() => {
        if (!gameOver || !gameStarted || settledRef.current) return;
    ...
    const handleSubmit = (e) => {
        ...
        const wasCorrect = trimmedInput && checkAnswer(trimmedInput, currentFlag, strictSpelling);
